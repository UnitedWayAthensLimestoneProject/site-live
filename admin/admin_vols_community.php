...
EOD;
    ...
    $('#editvoltable').w2grid({ 
		...
		/*	**ASU2016**  Added database fields for (disaster, community) 		6/19/2016		*/
            { field: 'community', caption: 'Community Volunteer', type: 'list', options: { items: [ 'YES', 'NO' ] } },		     
		...
		/*	**ASU2016**  Added database fields for (disaster, community, and ssn) that were missing in updates		6/19/2016		*/
            { field: 'community', caption: 'Community Volunteer', size: '140px', sortable: true, attr: 'align=center' },
		/* **ASU2016**  End added code 6/19/2016   */
 ],
        ...
        onEdit: function (event) {
			location.href = 'admin_vols_community.php?edit_vol='+event.recid;
        },
		...
		/* **ASU2016**  End added code 6/20/2016   */
			...
			/*	   "		  WHERE " . $where .	*/
				   "		  WHERE community = 1 " .
				   "	   ORDER BY community DESC, reg_date DESC, " .
				   "	 		    last_name";
		...
		{
		/*	**ASU2016**  Added database fields for (disaster, community, date_of_birth) 	6/20/2016		*/
			$javascript .= "{ recid: ".$row['vol_id'].", reg_date: '".date("n/j/Y",strtotime($row['reg_date']))."', first_name: '".$row['first_name']."', last_name: '".$row['last_name']."', email_address: '".$row['email_address']."', home_phone: '".$row['home_phone']."', cell_phone: '".$row['cell_phone']."', disaster: '".$row['disaster']."', community: '".$row['community']."', date_of_birth: '".date($row['date_of_birth']);
		/* **ASU2016**  End added code 6/20/2016   */
					
			if($row['community']==1)
			{
				$javascript .= "', community: 'YES' },";
			} 
			else 
			{
				$javascript .= "', community: 'NO' },";
			}
...
?>
		...
		<div id="admin_form_container">
			...
			<form name="form_search_for_volunteer" id="form_search_for_volunteer" method="post" style="margin:25px 10px" 
				action="admin_vols_community.php">
			</form>
			</center>
