...
EOD;
    ...
    $('#editvoltable').w2grid({ 
		...
		/*	**ASU2016**  Added database fields for (disaster, community) 		6/19/2016		*/
			{ field: 'age', caption: 'Age', type: 'int' },
		...
		/*	**ASU2016**  Added database fields for (disaster, community, and ssn) that were missing in updates		6/19/2016		*/
            { field: 'date_of_birth', caption: 'DOB', size: '100px', sortable: true,  attr: 'align=center' },
            { field: 'age', caption: 'Age', size: '100px', sortable: true, attr: 'align=center' },
		/* **ASU2016**  End added code 6/19/2016   */
 ],
        ...
        onEdit: function (event) {
			location.href = 'admin_vols_age.php?edit_vol='+event.recid;
        },
		...
		/* **ASU2016**  End added code 6/20/2016   */
				   "		   FROM volunteers " .
				   "		  WHERE " . $where .
				   "	   ORDER BY date_of_birth DESC, last_name, " .
				   "	 		    first_name";
		...
		/* **ASU2016**  End added code 6/20/2016   */
			...
			{
				$v_age = (date('Y-m-d'))-($row[date_of_birth]);
				$javascript .= "', age: $v_age },";
			} 
			else 
			{
				$javascript .= "', age: '0' },";
