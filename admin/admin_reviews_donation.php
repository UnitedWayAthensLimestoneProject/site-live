<?php 
	...
	if (isset($_POST['form_id'])) {
		...
		if ($searchform == 'submit_form_list_donation') {
			$searchforopp = 1;
			$searchoppid = $_POST['opp_id'];
	$javascript = <<<EOD
	$(document).ready(function() {
	
		$(' #form_list_donation ').hide();
		$(' #form_process_donation ').show();
		...
		$( '#form_process_donation' ).submit(function() 
		{
			return true;
		});
		
		$('#form_process_donation').validate( {
...
EOD;
		...
		$(document).ready(function() {
			...
			$(' #form_process_donation ').hide();
			$(' #form_list_donation ').show();
		
			$('#form_list_donation').validate( {
				errorPlacement: function(error, element) {
					if ( element.is(":radio") || element.is(":checkbox")) {
...
EOD;
    ...
    $('#editvoltable').w2grid({ 
        ...
        onAdd: function (event) {
			location.href = 'scripts/process_admin_donation_reviews.php?action=approve&id='+this.getSelection();
        },
        ...
        onDelete: function (event) {
			if(event.force==true){
				location.href = 'scripts/process_admin_donation_reviews.php?action=delete&id='+this.getSelection();
			}
		},
...
$javascript .= "		
	...
	page_start("United Way of Athens/Limestone County EMD Admin Page", $javascript, "adminReviewDonation",
...
?>
		...
		<div id="admin_form_container">
			<div class="form_description" align="center">
				<h2>Admin Review of New Donations</h2>
				<p>Gives Administrators the ability to approve/disapprove agency donation.
				<br>(NOTE: Only donation requiring review will appear here.)
				</p>
			</div>
