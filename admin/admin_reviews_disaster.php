<?php 
	...
	if (isset($_POST['form_id'])) {
		...
		if ($searchform == 'submit_form_list_disaster') {
			$searchforopp = 1;
			$searchoppid = $_POST['opp_id'];
	$javascript = <<<EOD
	$(document).ready(function() {
	
		$(' #form_list_disaster ').hide();
		$(' #form_process_disaster ').show();
		...
		$( '#form_process_disaster' ).submit(function() 
		{
			return true;
		});
		
		$('#form_process_disaster').validate( {
...
EOD;
		...
		$(document).ready(function() {
			...
			$(' #form_process_disaster ').hide();
			$(' #form_list_disaster ').show();
		
			$('#form_list_disaster').validate( {
				errorPlacement: function(error, element) {
					if ( element.is(":radio") || element.is(":checkbox")) {
...
EOD;
    ...
    $('#editvoltable').w2grid({ 
       ...
       onAdd: function (event) {
			location.href = 'scripts/process_admin_disaster_reviews.php?action=approve&id='+this.getSelection();
        },
        ...
        onDelete: function (event) {
			if(event.force==true){
				location.href = 'scripts/process_admin_disaster_reviews.php?action=delete&id='+this.getSelection();
			}
		},
...
$javascript .= "		
	...
	page_start("United Way of Athens/Limestone County EMD Admin Page", $javascript, "adminReviewDisaster",
...
?>
		...
		<div id="admin_form_container">
			<div class="form_description" align="center">
				<h2>Admin Review of New Disaster Reports</h2>
				<p>Gives Administrators the ability to approve/disapprove agency disaster.
				<br>(NOTE: Only disaster requiring review will appear here.)
				</p>
			</div>
