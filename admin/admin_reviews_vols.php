<?php 
	...
	if (isset($_POST['form_id'])) {
		...
		if ($searchform == 'submit_form_list_volunteer') {
			$searchforopp = 1;
			$searchoppid = $_POST['opp_id'];
	$javascript = <<<EOD
	$(document).ready(function() {
	
		$(' #form_list_volunteer ').hide();
...
EOD;
		...
		$(document).ready(function() {
			...
			$(' #form_list_volunteer ').show();
		
			$('#form_list_volunteer').validate( {
				errorPlacement: function(error, element) {
					if ( element.is(":radio") || element.is(":checkbox")) {
...
EOD;
    ...
    $('#editvoltable').w2grid({ 
        ...
        columns: [                
            { field: 'first_name', caption: 'First Name', size: '20%', sortable: true, attr: 'align=right' },
            { field: 'last_name', caption: 'Last Name', size: '20%', sortable: true, attr: 'align=right' },
            ...
            { field: 'reg_date', caption: 'Registration Date', size: '100px', sortable: true, searchable:'date', render:'date', attr: 'align=center' },
         ],
       onAdd: function (event) {
			location.href = 'scripts/process_admin_volunteer_reviews.php?action=approve&id='+this.getSelection();
        },
        ...
        onDelete: function (event) {
			if(event.force==true){
				location.href = 'scripts/process_admin_volunteer_reviews.php?action=delete&id='+this.getSelection();
			}
		},
       ...
       records: [
		...
		{
			$javascript .= "{ recid: ".$row['vol_id'].", reg_date: '".date("n/j/Y",strtotime($row['reg_date']))."', first_name: '".$row['first_name']."', last_name: '".$row['last_name']."', email_address: '".$row['email_address']."', home_phone: '".$row['home_phone']."', cell_phone: '".$row['cell_phone']."' },
			";
		}
...
?>
		...
		<div id="admin_form_container">
			<div class="form_description" align="center">
				<h2>Admin Review of New Volunteers</h2>
				<p>Gives Administrators the ability to approve/disapprove agency volunteer.
				<br>(NOTE: Only volunteer requiring review will appear here.)
				</p>
			</div>
